...
/**
 * *********************
 * This Class exports metadata concerning databases in CoDIMS system. You can
 * obtain metadata from the system catalog or from any other CODIMS' registered
 * database. At present, Metadata includes: - A list of registered databases
 * (names); - A list of TableMetadata objects from each database;
 *
 * @author fporto
...
public class DatabaseMetadata {
    ...
    /**
     * ************************
     * This method returns a list of TableMetaData objects. Each object contains
     * a table name and and a list of Column metadata objects. Tables can be
     * obtained from the catalog (through Catalog Manager) or from the
     * Transaction Monitor. Once the good Database metadata has been obtained,
     * we can build the list of table objects. The ListTableMetaData stores a
     * list of TableMetaData objects. To build TableMetadaData objects we use
     * the method getTables from the sql.DatabaseMetaData class. This method
     * retrieves table definitions based on a criteria comprising:catalog
     * name,schema name, table name pattern ( a string with special caracters
     * like (% or _) for generic queries) and types, that identify the type of
     * tables. In this application we require tables of type "TABLE". Next, we
     * look for the table primary key columns, using getPrimaryKey, and store in
     * a hashtable to be used as a lookup. Finaly, we fetch the corresponding
     * columns and build the ColumnMetaData. The list of objects of this type
     * are stored into the TableMetaData object.
     *
     * @param IRI
     * @return
     * @throws Exception
