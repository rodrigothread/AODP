...
/**
 * The TransactionMonitor manages the communication with the databases. It has a
 * singleton reference in Codims.
 *
 * The TransactionMonitor
 * <li> handles the transactions
 * <li> verfies the maximum number of connections for each database and the
 * connection timeouts
 * <li> closes the connections when necessary
 *
 * A user should call the TransactionMonitor in order to execute a query on a
 * database. The user first ask for a transaction id by calling the open()
 * method, passing the IRI of the database. Using this id, the user can execute
 * sql queries and get the results. The method executeQuery returns a ResultSet
 * object, it should be used with SELECT statement. For other statements use the
 * method execute(); this naming follows the one defined in Derby API. When the
 * connection is not needed anymore, it should be closed by calling the close()
 * method, passing the id of the transaction. If the connection is not used for
 * a certain amount of time (timeout set in the catalog), it is closed
 * automatically by the TransactionMonitor.
 *
 * Example :
 *
 * TransactionMonitor tm = TransactionMonitor.getTransactionMonitor(); int
 * transactionID = tm.open(myIRI); Query query = new Query;
 * query.setStringQuery("INSERT INTO myDB Values (1,1)");
 * tm.execute(transactionID,qusery); query.setStringQuery("SELECT * FROM myDB);
...
public class TransactionMonitor implements Runnable {
     ...
     * time maxBusyConnectionTime(set in the catalog in the initialConfig
     * table).
     */
    private Thread T;
    ...
    /**
     * Private constructor. It retrieves initial parameters from the catalog (as
     * the max busy connection time, thread sleep time), initializes the
     * ConnectionPoolManager and creates the thread that releases non-used
     * connections.
     *
    ...
    /**
     * This method opens a new transaction. It creates a new TransactionObject,
     * puts it in the transaction object list and returns a transaction id to
     * the caller.
     *
     * @param IRI - a database IRI.
     * @return - a newly created transaction id, to be used in further
     * communications with the TransactionMonitor.
     * @throws TransactionMonitorException
     */
    ...
    /**
     * The executeQuery method executes an sql query and returns a ResultSet
     * object. It should be used for executing SELECT statement.
     *
    ...
    /**
     * The execute method executes an sql query. It should be used for executing
     * statement different than SELECT (ex : INSERT, UPDATE, DELETE, DROP).
     *
     * @param id - The id of the transaction.
    ...
    /**
     * This method closes the TransactionMonitor. It closes all the opened
     * connection in the ConnecitonPoolManager and closes the checker thread.
     *
     * @throws TransactionMonitorException
     */
    ...
    /**
     * This method is called for starting the thread that checks non-used
     * connections in the transactionList hashtable. At a regular time interval
     * it calls the checkTransactionTimeOut method.
     */
