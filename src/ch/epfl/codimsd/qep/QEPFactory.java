...
/**
 * The QEPFactory is responsible for building QEP objects. It transforms what it
 * reads from formated QEP to hashtable of opNode structures.
 *
...
public class QEPFactory {
    ...
    /**
     * Retrieve the requested QEP path, according to its templateID and to the
     * requestType.
     *
     * @param requestType type of the request (@see
     * ch.epfl.codims.qeef.util.Constants).
    ...
    /**
     * Reads a qep file in xml or string format, and build the hashtable of
     * opNode structures representing the operators.
     *
     * If this method is called remotly, the parameter "type" should be set to
     * Constants.qepAccessTypeRemote; in order to read from a string file.
     *
     * @param qepFile path to QEP file.
     * @param type type of access (remote mode or centralized mode).
     * @param qep this qep. It is send in order to be filled with corresponding
     * hashtable.
    ...
    /**
     * Retrieve the IRI from which the operator of type "Scan" reads from. This
     * method is called in order to compute the initial number of tuples in the
     * DiscoveryOptimizer.
     *
    ...
    public static void logFile(String fileName, String fileContent) {
        ...
        try {
            ...
            if (fisrtCall == false) {
                ...
                if (!success) //logger.debug("Cannot create qeps directory (containing generated qeps).");
                {
                    fisrtCall = true;
                }
            }
    ...
    /**
     * Build a hashtable containing operator opNode structures, according to
     * what is defined in the query execution plan.
     *
