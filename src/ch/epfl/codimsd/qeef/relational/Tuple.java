 ...
 * Uma tupla � composta de um conjunto de valores que est�o associados cada um a
 * uma coluna definida nos metadados desta tupla.<p>
 * Os valores devem estar disposttos na mesma ordem em as colunad foram
 * definidas no metadado.
 *
 * 'The values must be disposed in the same order defined in metadata' 'One
 * tuple is composed of a set of values that are associates each one to a
 * definite column * in metadata of this tuple'
 *
...
public class Tuple extends Instance implements Serializable {
    ...
    /**
     * Lista com os valores de cada coluna. Os valores devem estar ordenados
     * pela posi��o da respectiva coluna nos metadados.
     *
     * List with the values of each column The values must be commanded for the
     * position of the respective column in the metadata ones
     *
     */
     ...
     *
     * It returns a collection with the values from each column Values meet
     * commanded according to metadata of the tuple
     *
     */
     ...
     *
     * Tuple attributes to values the columns of this. The values will keep the
     * original order
     *
     * @param values Valores a serem atribu�dos as colunas. Values to be
     * attributed the columns
     */
    ...
    /**
     * Atribui valor a primeira coluna que n�o possuir valor. It attributes to
     * value the first column that do not possess a value.
     *
    ...
    /**
     * Cria uma nova tupla que ser� o resultado da jun��o de left com right. It
     * creates a new tuple that will be the result of the junction of left with
     * right
     *
     ...
     *
     * It defines if one tuple must be serialized. It was opted to not using the
     * default implementation for performance reasons and memory occupation. The
     * default implementation keeps in memory serialized objects what it
     * generates errors if the volume of data manipulated will be very large.
     *
     * @param Fluxo para onde se deseja serializar o dados. for where if it
     * desires to serializar the data
     * @param metadata Metadados da tupla a ser serializada.
     *
     * @throws IOException Se acontecer algum problema durante grava��o dos
     * dados no fluxo.
     *
     ...
     *
     * @throws IOException Se acontecer algum problema durante leitura dos dados
     * no fluxo.
     * @throws EOFException Se chegou ao final do fluxo antes que a tupla fosse
     * completamente deserializada.
     *
    ...
    /**
     * Libera as refer�ncias feitas por esta tupla. the tuple liberates the
     * references made for this.
     */
    ...
    /**
     * Cria uma nova tupla id�ntica a esta. It creates new tupla identical to
     * this
     *
    ...
    /**
     * Grava os valores das colunas desta tupla no fluxo out para serem
     * visualizados.
     *
     * The tuple in the flow records the values of the columns of this out to be
     * visualized.
     *
     * @param out Fluxo para onde os valores ser�o gravados. Flow for where the
