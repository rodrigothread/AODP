 ...
 * operador de controle assincrono. Este operador consome os dados de forma
 * assincrona de seus produtores. Uma thread de consumo � inicializada para cada
 * operador qdo de sua inicializa��o. Os dados produzidos s�o deixados em um
 * buffer, de onde ser�o consumidos futuramente por
 * AsyncControlOperator#getNext(int)
 * .<p>
 * A etapa de open tamb�m � realizada de forma assincrona e depende de que um
 * produtor tenha a conclu�do para que o metadado deste operador possa ser
 * obtido e se encerre a inicializa��o.<p>
 *
 * Se alguma excess�o ocorrer durante o consumo dos dados pelas threads, essa
 * excess�o ser� registrada e lan�ada na pr�xima requisi��o por dados ao
 * operador.
 *
 * Changes made by Othman : - Logger static parameter. - Constructor changes,
 * we're not passing the blackBoard anymore. - deleted : int p =
 * getProducers().size();
 *
...
public abstract class AsyncControlOperator extends Operator {
     ...
     * S� depois poder� ser feito o encerramento dos demais produtores. O uso de
     * sem�foro � interessante em casos onde o processamento ainda n�o terminou
     * e o encerramento � solicitado. Desta forma, esta thread n�o ocupa
     * processamento e permite que as threads de consumo terminem seu trabalho.
     */
    ...
    /**
     * Determina quantas threads j� terminaram. Cada thread insere um null no
     * buffer quando termina processamento.
     */
     ...
     * @param blackBoard quadro de comunica��o utilizado pelos operadores de um
     * plano.
     ...
     *
     * @throws Exception Se algum erro que impossibiliza a execu��o acontecer
    ...
    /**
     * Consome uma tupla deste produtor de forma assincrona. Se houver tuplas
     * dispon�veis o resultado ser� imediato.
     *
     * @param consumerId Identificador do consumidor deste operador que
     * solicitou a opera��o.
     *
     * @return Tupla consumida. Null se n�o existir mais tuplas.
     *
     * @throws Exception Se acontecer algum erro durante a obten��o da tupla de
     * seu produtor.
     */
     ...
     * ser�o liberados.
     *
     * @throws Exception se acontecer algum durante a libera��o dos recursos.
     */
     ...
     * inseri-las no buffer para que possam ser consumidas futuramente. Os
     * valores null, que indicam que o operador n�o possui mais tuplas, n�o
     * devem ser inseridos no buffer. A implementa��o do operador define outros
     * meios para sinaliar o final de processamento. Com a inutiliza��o do
     * m�todo Thread#stop() do objeto thread, recomenda-se que de tempos em
     * tempos o procedimento verifique se o processo deve continuar existindo.
     * Nesta classe � definida a vari�vel continueProcessing que serve para este
     * fim. Ela possui valor true at� que a opera��o close seja executada.
     *
     * @param producer Produtor do qual os dados ser�o consumidos
     * @param buffer Buffer no qual as inst�ncias produzidas dever�o ser
     * inseridas.
     *
     * @throws Exception Se algum erro acontecer na produ��o dos dados.
     */
